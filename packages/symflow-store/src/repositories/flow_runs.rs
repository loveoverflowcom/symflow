...
pub async fn create_run(pool: &PgPool, flow_id: &str, initial_inputs: Option<Value>) -> Result<Uuid, StoreError> {
    let id = Uuid::new_v4();
    sqlx::query(
...
pub async fn set_run_status(pool: &PgPool, run_id: Uuid, status: RunStatus) -> Result<(), StoreError> {
    if matches!(status, RunStatus::Success | RunStatus::Failed) {
        sqlx::query("UPDATE flow_runs SET status = $1, finished_at = now() WHERE id = $2")
            .bind(status.as_db_str()).bind(run_id).execute(pool).await.map_err(backend_err)?;
    } else {
