...
pub async fn upsert_flow(pool: &PgPool, id: &str, name: &str, dsl_script: &str) -> Result<(), StoreError> {
    sqlx::query(
    ...
    ).bind(id).bind(name).bind(dsl_script).execute(pool).await.map_err(backend_err)?;
    Ok(())
}
...
pub async fn get_flow(pool: &PgPool, id: &str) -> Result<Option<FlowRecord>, StoreError> {
    ...
    ).bind(id).fetch_optional(pool).await.map_err(backend_err)?;
...
pub async fn list_flows(pool: &PgPool) -> Result<Vec<FlowSummary>, StoreError> {
    ...
    ).fetch_all(pool).await.map_err(backend_err)?;
