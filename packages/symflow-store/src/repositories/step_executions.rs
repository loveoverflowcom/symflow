...
pub async fn upsert_step(pool: &PgPool, run_id: Uuid, step_id: &str, status: StepStatus) -> Result<(), StoreError> {
    sqlx::query(
...
pub async fn set_step_status(pool: &PgPool, run_id: Uuid, step_id: &str, status: StepStatus) -> Result<(), StoreError> {
...
pub async fn set_resolved_inputs(pool: &PgPool, run_id: Uuid, step_id: &str, inputs: &Value) -> Result<(), StoreError> {
    sqlx::query("UPDATE step_executions SET resolved_inputs = $1 WHERE run_id = $2 AND step_id = $3")
        .bind(inputs).bind(run_id).bind(step_id).execute(pool).await.map_err(backend_err)?;
    Ok(())
}

pub async fn complete_step(pool: &PgPool, run_id: Uuid, step_id: &str, outputs: &Value) -> Result<(), StoreError> {
...
pub async fn fail_step(pool: &PgPool, run_id: Uuid, step_id: &str, error: &str) -> Result<(), StoreError> {
...
pub async fn get_step_outputs(pool: &PgPool, run_id: Uuid, step_id: &str) -> Result<Option<Value>, StoreError> {
    let row = sqlx::query("SELECT outputs FROM step_executions WHERE run_id = $1 AND step_id = $2")
        .bind(run_id).bind(step_id).fetch_optional(pool).await.map_err(backend_err)?;
    match row {
        Some(r) => Ok(r.try_get::<Option<Value>, _>("outputs").map_err(backend_err)?),
        None    => Ok(None),
    }
}

pub async fn append_agent_log(pool: &PgPool, run_id: Uuid, step_id: &str, entry: Value) -> Result<(), StoreError> {
...
pub async fn list_steps(pool: &PgPool, run_id: Uuid) -> Result<Vec<StepRecord>, StoreError> {
    ...
    ).bind(run_id).fetch_all(pool).await.map_err(backend_err)?;
    Ok(rows.into_iter().map(StepExecRow::into_record).collect())
}

pub async fn get_step(pool: &PgPool, run_id: Uuid, step_id: &str) -> Result<Option<StepRecord>, StoreError> {
