...
pub async fn run_flow(
    ...
    for step_id in &compiled.order {
        store
            .upsert_step(run_id, step_id, StepStatus::Pending)
            .await?;
    }
    store.set_run_status(run_id, RunStatus::Running).await?;
    emit(
        ctx.bus.as_ref(),
        RunEvent::run_status(run_id, RunStatus::Running.as_db_str()),
    );
    ...
    for step_id in &compiled.order {
        ...
        let resolved = match resolver::resolve_value(&step.with, run_id, store).await {
            ...
            Err(e) => {
                any_failed = true;
                fail(store, ctx, &mut statuses, step_id, &e.to_string()).await?;
                continue;
            }
        };
        ...
        let Some(step_type) = step.step_type() else {
            ...
            fail(
                ...
                &mut statuses,
                step_id,
                &format!("loại bước không hợp lệ: '{}'", step.kind),
            )
            .await?;
            continue;
        };

        let resolved = if step_type == StepType::ManualTrigger {
            crate::nodes::manual_trigger::apply_initial_inputs(resolved, &ctx.initial_inputs)
        } else {
            resolved
        };
        store
            .set_step_resolved_inputs(run_id, step_id, &resolved)
            .await?;
        ...
        match outcome {
            Ok(outputs) => {
                store.complete_step(run_id, step_id, &outputs).await?;
                record(&mut statuses, ctx, step_id, StepStatus::Completed);
            }
            Err(err) => {
                any_failed = true;
                fail(store, ctx, &mut statuses, step_id, &err).await?;
            }
        }
    }
    ...
    emit(
        ctx.bus.as_ref(),
        RunEvent::run_status(run_id, final_status.as_db_str()),
    );
    Ok(final_status)
}
...
fn record(
    ...
    emit(
        ctx.bus.as_ref(),
        RunEvent::step_status(ctx.run_id, step_id, status.as_db_str()),
    );
}
...
mod tests {
    ...
    #[tokio::test]
    async fn manual_trigger_initial_inputs_override_defaults_and_resolve_downstream() {
        let flow = Flow {
            flow_id: "manual-input-override".to_string(),
            name: String::new(),
            steps: vec![
                Step {
                    id: "receive_url".to_string(),
                    needs: vec![],
                    kind: "manual_trigger".to_string(),
                    with: json!({ "url": "https://example.com" }),
                },
                Step {
                    id: "echo_url".to_string(),
                    needs: vec!["receive_url".to_string()],
                    kind: "manual_trigger".to_string(),
                    with: json!({ "seen_url": "{{steps.receive_url.url}}" }),
                },
            ],
        };
        let store = MemoryStore::new();
        let initial_inputs = json!({ "url": "https://vnexpress.net" });
        let run_id = store
        ...
        let status = run_flow(&flow, &ctx, &store).await.unwrap();

        assert_eq!(status, RunStatus::Success);
        let receive_url = store
            .get_step(run_id, "receive_url")
            .await
            .unwrap()
            .unwrap();
        assert_eq!(
            receive_url.resolved_inputs.unwrap()["url"],
            "https://vnexpress.net"
        );
        assert_eq!(receive_url.outputs.unwrap()["url"], "https://vnexpress.net");

        let echo_url = store.get_step(run_id, "echo_url").await.unwrap().unwrap();
        assert_eq!(
            echo_url.resolved_inputs.unwrap()["seen_url"],
            "https://vnexpress.net"
        );
        assert_eq!(
            echo_url.outputs.unwrap()["seen_url"],
            "https://vnexpress.net"
        );
    }
}
