...
impl Store for MemoryStore {
    ...
    async fn delete_flow(&self, id: &str) -> Result<bool, StoreError> {
        ...
        if existed {
            ...
            inner
                .steps
                .retain(|(run_id, _), _| !removed_run_ids.contains(run_id));
        }
        Ok(existed)
    }
    async fn create_run(
    ...
    async fn set_step_status(
        ...
        if let Some(s) = self
            ...
            .unwrap()
            .steps
            .get_mut(&(run_id, step_id.to_string()))
        {
            s.status = status;
    ...
    async fn set_step_resolved_inputs(
        ...
        if let Some(s) = self
            ...
            .get_mut(&(run_id, step_id.to_string()))
        {
            s.resolved_inputs = Some(inputs.clone());
        }
        Ok(())
    }
    ...
    async fn append_agent_log(
            ...
            .get_mut(&(run_id, step_id.to_string()))
        {
            ...
            if let Value::Array(ref mut v) = arr {
                v.push(entry);
            }
        }
        Ok(())
